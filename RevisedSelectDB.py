...
class RevisedSelectDB:
    def select_data_from_DB(self, setting):
        select_query = ('''
            ...
            AND A_external_edges = %s AND B_external_edges = %s 
            AND A_initial_state = %s AND B_initial_state = %s
            AND A_node_number = %s AND B_node_number = %s;''' % (str(setting.table), str(setting.Structure),
                                                                 int(setting.A_edge), int(setting.B_edge),
                                                                 int(setting.A_inter_edges),
                                                                 int(setting.B_inter_edges),
                                                                 float(setting.A_initial_state),
                                                                 float(setting.B_initial_state),
                                                                 int(setting.A_node), int(setting.B_node)))
