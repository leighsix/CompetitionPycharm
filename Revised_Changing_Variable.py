...
class Revised_Changing_Variable:
    ...
    def calculate_and_input_database(self, setting, beta):
        self.revised_repeat_dynamics.repeat_dynamics(setting, beta)
        panda_db = self.mp.making_dataframe_per_step(setting, self.revised_repeat_dynamics.Num_Data)
        print(panda_db.loc[0])
        panda_db['beta'] = beta
        engine = sqlalchemy.create_engine('mysql+pymysql://root:2853@localhost:3306/renew_competition')
        panda_db.to_sql(name='revised_layer_state', con=engine, index=False, if_exists='append')
    ...
    def paralleled_work(self, setting):
        ...
        for i in setting.D:
            setting_variable_list.append(i)
